...
public class CollideHelper {
    ...
    public static boolean aabbPolygon(AABB a, Polygon b) {
        ...
        List<Vector2f> v = new ArrayList<>(4);
        ...
        v.add(new Vector2f(x1, y1));
        v.add(new Vector2f(x2, y1));
        v.add(new Vector2f(x2, y2));
        v.add(new Vector2f(x1, y2));

        return sat(v, b.getPoints()) || sat(b.getPoints(), v);
    }

    public static boolean circleCircle(Circle a, Circle b) {
        float d = a.getCenter().distanceSquared(b.getCenter());
    ...
    public static boolean circlePolygon(Circle c, Polygon b) {
        List<Vector2f> points = b.getPoints();
    ...
    private static boolean sat(List<Vector2f> pointsA, List<Vector2f> pointsB) {
        Vector2f proj = new Vector2f();

        for (int i = 0; i < pointsA.size(); i++) {
            Vector2f a1 = pointsA.get(i);
            Vector2f a2 = pointsA.get((i + 1) % pointsA.size());
    ...
    public static boolean circleEdge(Circle c, float x1, float y1, float x2, float y2) {
        Vector2f d = new Vector2f(x2, y2).sub(x1, y1);
        Vector2f e = new Vector2f(x1, y1).sub(c.getCenter());
    ...
    public static boolean circlePoint(Circle c, float x, float y) {
        return c.getCenter().distanceSquared(x, y) <= c.getRadiusSquared();
    }
}
